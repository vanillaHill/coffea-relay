import { IsString, IsNumber, IsOptional, IsEthereumAddress, IsHexadecimal } from 'class-validator';
...
export class SubmitTransactionDto {
  ...
  @ApiProperty({ description: 'Gas limit for transaction', required: false, example: 500000 })
  @IsOptional()
  @IsNumber()
  gasLimit?: number;
  ...
  @ApiProperty({ description: 'Max priority fee per gas (EIP-1559)', required: false })
  @IsOptional()
  @IsString()
...
export class TransactionResponseDto {
  ...
  @ApiProperty({ description: 'Additional information or error message', required: false })
  message?: string;
}
...
export class TaskStatusDto {
  ...
  @ApiProperty({ description: 'Current task status', enum: ['pending', 'submitted', 'success', 'failed', 'cancelled'] })
  status: string;

  @ApiProperty({ description: 'Transaction hash when submitted', required: false })
